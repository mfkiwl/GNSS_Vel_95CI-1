...
def cal_95CI(year,ts,GNSS,outDir,DIR,output,pltshow):
    """
    ...
    outDir: directory of all output files
    DIR: string, NS or EW or UD component
    output: 'on' or 'off'. 'on' will plot figures (*.png, *.pdf) and write out data (*.txt), saved in "outDir".
    ...
    # get the pass
    path = os.getcwd()  
    xdir=os.path.abspath(os.path.join(path, os.pardir))+'/'    
    print(xdir)
...
# -----------------------------------------------------------------------------
    if output == 'on':
       ...
       y.to_csv(outDir+GNSS +'_'+ DIR +'_ACF.txt', index = True, header=True)
       ...
       plt.savefig(outDir+GNSS +'_'+ DIR + "_ACF.pdf")
       plt.savefig(outDir+GNSS +'_'+ DIR + "_ACF.png")
      
       # Plot decompositions
       fig, (fig1,fig2,fig3,fig4) = plt.subplots(4, figsize=(16,14))
       fig.subplots_adjust(hspace=0.3)
       fig.suptitle('Decomposition of GNSS-Derived Daily Displacement Time Series: '+ GNSS + '-' + DIR, size=14,  y=0.93);
       ...
       fig.savefig(outDir+GNSS + '_' + DIR + '_Decomposition.png')
       fig.savefig(outDir+GNSS + '_' + DIR + '_Decomposition.pdf')
       
       if pltshow == 'on':
          plt.show()

       # output the time series, original and filled
       f1_out = outDir+GNSS + "_" + DIR + "_Linear_NonLinear.txt"
       # build the DataFrame
       ...
       ri=pd.DataFrame(ri)
       f2_out = outDir+GNSS + "_" + DIR + "_SeasonalM.txt"
